...
Atoms.readCif = function(cif, symtol=1e-3) {
    ...
    for (var sname in structs) {
        ...
        for (var i = 0; i < asites.length; ++i) {
            // if type_symbol is not present, split label using regex and use
            // first part as symbol
            var symbol = '';
            if (asites[i].type_symbol === undefined) {
                // split by first non-letter character
                symbol = asites[i].label.split(/[^a-zA-Z]+/)[0];
            } else {
                symbol = asites[i].type_symbol;
            }
            // If symbol is still empty here, throw an error
            if (symbol === '') {
                throw Error('Could not determine symbol for atom ' + asites[i]);
            }
            symbols.push(symbol);
